...
df_geneCounts = pd.read_csv("./snp_files/normal_barcodes/bilan_files/only_tumor/"+cancer_file_name, sep="\t", index_col=0)
df_geneCounts = df_geneCounts[df_geneCounts.sum(axis = 1) > 0] #we need to remove rows where all samples are at 0
df_geneCounts = df_geneCounts.T #we need the genes as columns 
...
df_snp_raw = pd.read_csv("./snp_files/tcga_snp_exonhancers_allall.tsv", sep="\t", header=None)

df_snp = df_snp_raw[df_snp_raw.iloc[:, 17] == cancer_type] #filter snp on cancer 

df_snp_grouped = df_snp.groupby(3)

df_final = pd.DataFrame(columns=["index","baseMean","log2FoldChange","lfcSE","stat","pvalue","padj","nlog10","exon","color"])
...
for group_name, group_data in df_snp_grouped: #FOR EACH EXON IN SNP FILE 
    ...
    for patient in dic_all_snp_patient_by_exons[cancer_type][group_name]:
    ...
    # valid_target = [index for index in list_of_target if index in res.index]
    # res_target = res.loc[valid_target]
    # res_target = res_target.reset_index()
    # res_target["exon"] = group_name
