...
with open(file) as f:
    ...
    old_row = {"Chrom":"","Start":"","End":"","Exon_ID":"","Score":"","Strand":"", "density_peak_50pb":""}

    for line in lines:
        # check if it was the cds
        density_peak_50pb = line.strip().split()[9]
        ...
        if first == True: #if first line of file
            ...
            for key,values in old_row.items():
                if key == "density_peak_50pb":
                    old_row[key] = density_peak_50pb
                else:
        ...
        if chrom == old_row["Chrom"] and start == old_row["Start"] and end == old_row["End"] and gene == old_row["Exon_ID"]:
            if int(old_row["density_peak_50pb"]) > int(density_peak_50pb):
                continue
            else:
                old_row["density_peak_50pb"] = density_peak_50pb
...
df.to_csv('exons_ovlpTF_NR_cCREs_Fantom5+Enh_NonCoding_cdsStartEnd_UTR5_UTR3_peakDensity_density50pb_Hsap.bed', sep='\t', index=False, header=False)