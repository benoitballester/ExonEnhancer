...
for group_name, group_data in grouped_df:
    ...
    for index, row in group_data.iterrows():
        ...
        if row[13] not in id_list:
            id_list.append(row[13])
        
        if dic_atlas_cell[row[13]] not in cell_dic:
            cell_dic[dic_atlas_cell[row[13]]] = int(row[14])
        else:
            if int(row[14]) > cell_dic[dic_atlas_cell[row[13]]]:
                cell_dic[dic_atlas_cell[row[13]]] = int(row[14])
